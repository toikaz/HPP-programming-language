...
- Что бы сделать исполняемый файл из .hpp нужно написать в консоли hppinstaller <полный путь к папке с файлом (без слеша в конце)> <имя файла.vvm> , тогда скрипт создаст папку которая важна для работы программы и исполняемый батник который будет запускать скрипт на ос на которой может быть не установлен HPP (важно что бы файл и батник были в одной директории).
...
- Еще можно открыть с помощью C:\windows\system32\hpp.exe для ".hpp" или с помощью C:\windows\system32\vvm.exe для ".vvm".

## Команды для консольного приложения (HPP)

### Базовые команды

- **str \ int \ float <имя> = <значение>**: объявляет переменную и присваивает значение.
  - Пример:
    ```HPP
    str a = "Hello!"
    int b = 5
...
- **int \ float <имя> = Random <значение 1> <значение 2>**: объявляет переменную и присваивает ей рандомное значение в диапозоне 1 значения до 2 значения.
  - Пример:
    ```HPP
    int b = Random 1 3
    System.out.print %b%
...
- **int \ float <имя> = $time$**: объявляет переменную и сохраняет текущее время с точностью до секунды.
  - Пример:
    ```HPP
    int b = $time$
    ```

- **int \ float <имя> = $date$**: объявляет переменную и сохраняет дату.
  - Пример:
    ```HPP
    int b = $date$
    ```

- **System.ReadLine() \ System.ReadInt() \ System.ReadFloat()**: считывает ввод пользователя.
  - Пример:
    ```HPP
...
- **dir <путь по желанию>**: команда dir из windows.
  - Пример:
    ```HPP
    str a = "C:\"
    dir %a%
...
- **System.add_image.window <имя_окна> <координата> <координата> <ширина> <высота> <путь к файлу .png>**: добавляет в окно изображение пнг
  - Пример:
    ```HPP
    System.add_image.window window 0 0 100 100 C:\users\user\desktop\test.png
    ```
...
- **System.clear_buttons.window <имя_окна>**: очищает все кнопки в окне кроме текста и ввода информации
  - Пример:
    ```HPP
    System.clear_buttons.window Calculator
    ```
    
- **System.clear_images.window <имя_окна>**: очищает все изображения в окне
  - Пример:
    ```HPP
    System.clear_images.window Calculator
    ```
...
- **System.keyboard.hold <число> <имя кнопки>**: зажимает кнопку на указанное количество времени
- **System.keyboard.read <имя переменной>**: считывает кнопку и сохраняет в переменную
...
- **array indexoutln <имя_массива> <индекс>**: Выводит элемент массива по указанному индексу в консоль в одной строке что бы можно было вывести несколько объектов на одной строке (если вывести несколько объектов в одну строку таким способом то они будут выводиться слитно). Индексация начинается с 0. Индекс может быть указан как число или как переменная (используя `%`).

- **array indexprintln <имя_массива> <индекс>**: Выводит элемент массива по указанному индексу в консоль в одной строке что бы можно было вывести несколько объектов на одной строке (если вывести несколько объектов в одну строку таким способом то они будут выводиться раздельно). Индексация начинается с 0. Индекс может быть указан как число или как переменная (используя `%`).
...
- P.S - если вы хотите конвертировать float в int или наоборот то напишите: 
  ```HPP
  int integer = %float% - 0
